...
def _with_retry(operation,attempts=3,base_delay=0.5):
    for attempt in range(attempts):
        try: return operation()
            ...
            if isinstance(error, (InputErrors, InvalidSignatureError, ValueError)): raise
...
class StarkClient:
    ...
    def issue_batch(self,minimum=8,maximum=12,idempotency_key=None):
        ...
        result=[]
        size=minimum + int(hashlib.sha256(batch_key.encode()).hexdigest()[:8],16) % (maximum-minimum+1)
        for index in range(size):
            request_key=f"{batch_key}:{index}"
            state=self.store.claim_invoice_creation(request_key,datetime.now(timezone.utc).isoformat())
            ...
            def create_or_recover():
                ...
                return existing or starkbank.invoice.create([starkbank.Invoice(amount=random.randint(self.settings.invoice_min_amount,self.settings.invoice_max_amount),name=random.choice(["Ana Silva","Bruno Costa","Carla Souza","Diego Oliveira"]),tax_id=_random_cpf(),due=datetime.now(timezone.utc)+timedelta(hours=3),expiration=10800,tags=["starkbank-trial",tag])])
            invoices=_with_retry(create_or_recover)
            invoice=invoices[0]
            self.store.save_invoice(invoice.id,datetime.now(timezone.utc).isoformat())
            self.store.complete_invoice_creation(request_key,invoice.id,state["lease_token"])
    ...
    def transfer_paid_invoice(self,invoice_id,event_invoice):
        ...
        if not claim["claimed"]:
            if claim["status"]=="completed": return None
            raise RuntimeError("transfer lease is active")
        try:
            receipt=receipt_from(invoice_id,_with_retry(lambda: starkbank.invoice.payment(invoice_id)),event_invoice)
            response=_with_retry(lambda: starkbank.transfer.create([starkbank.Transfer(amount=receipt.net_amount,external_id=f"starkbank-trial:{invoice_id}",**DESTINATION)]))
            transfer=response[0]
            self.store.complete(invoice_id,receipt.net_amount,getattr(transfer,"id",""),claim["lease_token"]); return receipt.net_amount
        except Exception:
            try: self.store.mark_retryable(invoice_id,claim["lease_token"])
            finally: raise
