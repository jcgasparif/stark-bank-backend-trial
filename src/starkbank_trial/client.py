...
class StarkClient:
    ...
    def issue_batch(self,minimum=8,maximum=12):
        ...
        for _ in range(random.randint(minimum,maximum)):
            invoice=starkbank.invoice.create(starkbank.Invoice(amount=random.randint(self.settings.invoice_min_amount,self.settings.invoice_max_amount),name=random.choice(["Ana Silva","Bruno Costa","Carla Souza","Diego Oliveira"]),tax_id=str(random.randint(10000000000,99999999999)),due=datetime.now(timezone.utc)+timedelta(hours=3),expiration=10800,tags=["starkbank-trial"]))
    ...
    def transfer_paid_invoice(self,invoice_id,event_invoice):
        ...
        try:
            ...
            transfer=response[0] if isinstance(response,list) else response
