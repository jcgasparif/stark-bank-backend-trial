...
class Store:
    """SQLite implementation used when the application runs locally."""

    LEASE_SECONDS = 120

    def __init__(self, path: Path):
        # Create the local data directory and all tables on first use.
        self.path = path
    ...
    def save_invoice(self, invoice_id, created_at):
        """Store an invoice once; repeated writes are ignored."""
        with self._db() as db:
            db.execute(
    ...
    def save_event(self, event_id, invoice_id, received_at):
        """Store a webhook once so duplicate deliveries are traceable."""
        with self._db() as db:
            db.execute(
    ...
    def claim(self, invoice_id, created_at):
        """Acquire an invoice transfer lease or report its current state."""
        now = int(time.time())
        lease_token = str(uuid.uuid4())
        with self._db() as db:
            db.execute("BEGIN IMMEDIATE")
            # The write lock makes the read-and-claim decision atomic locally.
            row = db.execute(
    ...
    def mark_retryable(self, invoice_id, lease_token):
        """Release a failed transfer for a later retry."""
        with self._db() as db:
            cursor = db.execute(
                """
                UPDATE claims
    ...
    def complete(self, invoice_id, amount, stark_id, lease_token):
        """Mark a transfer as completed only for its lease owner."""
        with self._db() as db:
            cursor = db.execute(
                """
                UPDATE claims
    ...
    def claim_invoice_creation(self, request_key, created_at):
        """Acquire the lease protecting one invoice creation request."""
        now = int(time.time())
        lease_token = str(uuid.uuid4())
        with self._db() as db:
            db.execute("BEGIN IMMEDIATE")
    ...
    def complete_invoice_creation(self, request_key, invoice_id, lease_token):
        """Save the created invoice ID and release its creation lease."""
        with self._db() as db:
            cursor = db.execute(
                """
