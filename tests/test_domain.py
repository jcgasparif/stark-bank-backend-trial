...
def test_destination():
    assert (
        DESTINATION["bank_code"] == "20018183"
        and DESTINATION["account_type"] == "payment"
    )
...
def test_scheduler():
    calls = []
    run_for_24_hours(
        type("C", (), {"issue_batch": lambda self, a, b: calls.append((a, b))})(),
        sleep=lambda _: None,
    )
    assert len(calls) == 8 and all(x == (8, 12) for x in calls)
